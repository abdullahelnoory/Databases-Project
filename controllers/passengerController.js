...
exports.getTrips = async (req, res) => {
    ...
    const p_id = req.body.p_id;
    const query = 'select d1.ssn, d1.fname driver_fname, d1.mname, d1.lname, s.station_id, s.station_name source, s.street, s.governorate, d.station_id, d.station_name destination, d.street, d.governorate, t.trip_id , t.price , t.estimated_time from "Driver" d1, "Station" s, "Station" d, "Trip" t where d1.ssn = t.d_ssn and t.source_station = s.station_id and t.destination_station = d.station_id and t.status = $1 and t.trip_id not in (select t_id from "Passenger Trip" where p_id = $2)';
    try
    {
...
exports.getStatusPrivateTrips = async (req, res) => {
    const p_id = req.body.p_id;
    const query = 'select source, destination, d_ssn from "Private Trip" where p_id = $1';
    try
    {
        const result = await pool.query(query, [p_id]);
        res.json({success : true, data : result.rows});
